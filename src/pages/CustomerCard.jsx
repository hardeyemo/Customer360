        ...
        <div className="relative bg-white rounded-2xl shadow-md p-4 flex flex-col justify-between space-y-6 hover:shadow-lg transition-shadow duration-300 w-full sm:w-auto">
            <div className="absolute top-4 right-4 sm:hidden" ref={dropdownRef}>
                <button className="text-gray-500 hover:text-gray-700 transition-colors duration-200"
                    onClick={() => setDropdownOpen((prev) => !prev)} ><HiOutlineDotsHorizontal size={22} />
                </button>
                {dropdownOpen && (
                    <ul className="absolute right-0 mt-2 w-40 bg-white border border-blue-200 rounded shadow-md z-20">
                        <li className="px-4 py-2 hover:bg-blue-50 cursor-pointer text-gray-700" onClick={() => handleAction("Edit")}  >Edit </li>
                        <li className="px-4 py-2 hover:bg-blue-50 cursor-pointer text-gray-700" onClick={() => handleAction("Delete")}>Delete </li>
                        <li className="px-4 py-2 hover:bg-blue-50 cursor-pointer text-gray-700" onClick={() => handleAction("Send Message")}> Send Message</li>
                    </ul>
                )}
            </div>
            <div className="space-y-2">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                    <h2 className="text-lg sm:text-xl font-semibold text-gray-800 truncate"> {customer.name}</h2>
                    <span className={`inline-flex self-start text-xs font-medium px-3 py-1 rounded-full transition-colors duration-200 
                        ${customer.status === "Active" ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-600"}`} > {customer.status}
                    </span>
                </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-2 mt-4">
                <button onClick={() => navigate(`/customer/${customer.id}`)}
                    className="flex items-center justify-center gap-2 bg-purple-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-full hover:bg-purple-600 transition-colors duration-200 w-full sm:w-auto text-sm sm:text-base">  <FiUser /> View Profile
                </button>
